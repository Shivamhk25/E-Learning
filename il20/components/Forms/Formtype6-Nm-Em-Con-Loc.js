...
import { CheckIfUTMIsOTPEnabled } from '../../services/helper'
...
const DynamicMxProgram3Form = ({
  toggleModal,
  mxProgram3Value = mxProgram3Value || '', //eg. IIM EPCBIM or CIBOP etc
  leadsquareSourceKey=leadsquareSourceKey
}) => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('leadSource', leadsquareSourceKey.length > 0 ? leadsquareSourceKey :leadSource)
    console.log('leadScore', leadSource)
    ...
    if (mxProgram3Value) {
      setAreaofinterest(mxProgram3Value)
    }
  }, [])
  ...
  const submitHandler = async (e) => {
    ...
    try {
      ...
      const data = await leadApiHelper({
        ...
        mx_Program_from_website: areaofinterestValue ? areaofinterestValue : '',
        mx_Program_3: areaofinterestValue ? areaofinterestValue : '',
      })
...
export default DynamicMxProgram3Form
